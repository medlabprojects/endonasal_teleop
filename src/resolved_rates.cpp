...
********************************************************************************/
    // TELEOP PARAMETERS
    double scale_factor = 0.10;		// originally 0.3
    double weight_master_scale = 1.0; // originally 1.0e6
    double lambda_tracking = 1.0e8; 	// originally 1.0
    double lambda_damping = 0.1;    	// originally 5.0
    ...
    W_damping(3,3) = lambda_damping*weight_master_scale*5e8;
    W_damping(4,4) = lambda_damping*weight_master_scale*5e8;
    W_damping(5,5) = lambda_damping*weight_master_scale*5e8;
...
********************************************************************************/
    ...
    while (ros::ok())
    {
		...
		enc2.count[1] = 0;
 		enc2.count[2] = 0; //(int)((q_vec[5] - qstart.Beta[2]) * scale_trans_outer); 	// outer translation;
		enc2.count[3] = 0;
 		enc2.count[4] = 0;
  		enc2.count[5] = 0;

		//std::cout << "enc1: " << enc1 << std::endl << std::endl;
		//std::cout << "enc2: " << enc2 << std::endl << std::endl;
